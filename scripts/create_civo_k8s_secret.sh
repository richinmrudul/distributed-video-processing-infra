...
ENV_FILE="${CIVO_ENV_FILE:-k8s/civo.env}"
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  echo "Copy k8s/civo.env.example to $ENV_FILE and replace every placeholder first."
  exit 1
fi
...
secret_env_file="$(mktemp -t civo-k8s-secret-XXXXXX.env)"
cleanup() {
