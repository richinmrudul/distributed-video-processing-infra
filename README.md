...
- **Presigned assets (object mode, after COMPLETED):** `curl -sS http://localhost:8000/api/v1/videos/{id}/assets | jq .` — temporary download URLs for raw/processed/thumbnail (expire after 1 hour by default). URLs use `OBJECT_STORAGE_PUBLIC_ENDPOINT` (default `http://localhost:9000`) so browsers on the host can reach MinIO.
